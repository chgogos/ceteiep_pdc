...
unsigned char** FlipImageV(unsigned char** img)
{
	...
	//vertical flip
	#pragma omp parallel for num_threads(NumThreads) private(pix, row, col)
...
int main(int argc, char** argv)
{
	...
	printf("\n\nTotal execution time: %9.4f ms",timer*1000);
	printf(" (%7.3f ns per pixel)\n", 1000000000*timer/(double)(ip.Hpixels*ip.Vpixels));
